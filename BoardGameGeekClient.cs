...
using BoardGameGeekClientAPI.Models;
using System;
...
namespace BoardGameGeekClientAPI
{
    public class BoardGameGeekClient
    {
        ...
        public async Task<GameDetails> LoadGame(int GameId, bool useLongCache)
        {
            ...
            if (useLongCache)
            {
                details = _gameCache.Get(CacheHelper.LongThingKey(GameId)) as GameDetails;
            }
            ...
            try
            {
                ...
                if (details != null)
                {
                    _gameCache.Set(CacheHelper.LongThingKey(details.GameId), details, DateTimeOffset.Now.AddSeconds(GameCacheDuration));
                }
